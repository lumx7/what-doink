...
const char tracking_left_port = 'G', tracking_right_port = 'A', tracking_middle_port = 'E';

const ChassisScales ctrackingScales{
    { 2.7668 * inch, 15.5 * inch}, // wheel diam, wheelbase diam
...
Chassis::Chassis() :
    fl_motor(FL_PORT, pros::E_MOTOR_GEARSET_18, true),
    fr_motor(FR_PORT, pros::E_MOTOR_GEARSET_18),
    bl_motor(BL_PORT, pros::E_MOTOR_GEARSET_18),
    br_motor(BR_PORT, pros::E_MOTOR_GEARSET_18, true),

    odometry(trackingScales, tracking_left, tracking_right, inertial),
...
void Chassis::oc_update(int32_t joy_y, int32_t joy_a, bool brake_mode) {
    ...
    */
    ...
    if (brake_mode) {
        ...
        // if (abs(brv) < 1)
        //     brv = 0;
        fl_motor.move_velocity((x * 100) / 12000);
        fr_motor.move_velocity((y * 100) / 12000);
        bl_motor.move_velocity((x * 100) / 12000);
        br_motor.move_velocity((y * 100) / 12000);
    } else {
        fl_motor.move_voltage(x*94);
        fr_motor.move_voltage(y*94);
        bl_motor.move_voltage(x*94);
        br_motor.move_voltage(y*94);
    }
};
